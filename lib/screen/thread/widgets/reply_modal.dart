...
class ReplyRoute extends PopupRoute {
  ...
  @override
  Duration get transitionDuration => const Duration(microseconds: 750);
}
              ...
              children: [
                ...
                IconButton(
                  onPressed: () async {
                    ...
                    if (image != null) {
                      final uid = profile.memberUid;
                      if (uid == '0') {
                        return;
                      }
                      final aid = await keylol.checkPost().then((resp) {
                        final uploadHash = resp.variables.allowPerm.uploadHash;
                        return keylol.forumUpload(uid, uploadHash, image);
                      });
